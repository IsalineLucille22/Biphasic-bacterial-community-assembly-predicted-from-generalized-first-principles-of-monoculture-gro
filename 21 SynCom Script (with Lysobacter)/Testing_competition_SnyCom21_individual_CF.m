...
%Loadind data
Parameters_set = readtable(strcat('Data/','MergedData_copy.xlsx'), 'Sheet', 8,'Range','48:69', 'Format','auto');
...
writetable(ind_syncom_test_Cliff_1wk_Fig2, 'Data/ttest_stat_Fig2.csv');
...
name_Exp = 'Zenodo_Senka_Average_Interaction';
%actual absolute abundance data, has quadriplicate values for each time point
Data_Evol_21 = readtable(strcat('Data/','S20_S21_abs_abund_cfu_Senka.xlsx'), 'Sheet', 7, 'Range','1:22', 'Format','auto'); %Senka's data
Data_Evol_20 = readtable(strcat('Data/','S20_S21_abs_abund_cfu_Senka.xlsx'), 'Sheet', 4, 'Range','1:21', 'Format','auto');
Parameters_set = readtable(strcat('Data/','MergedData_copy.xlsx'), 'Sheet', 8,'Range','48:69', 'Format','auto');
...
for i = 1:(nb_rep_21 - 1)
    ...
    % scatter(flip(Data_Evol_21_wo_Lysobacter_end(:,i)),1:S_wo_Lysobacter, 50, '.', 'r')
end
%Automatized this
for i = 1:(nb_rep_20 - 1)
    scatter(flip(log10(Data_Evol_20_wo_Lysobacter_end(:,i)./(single_cell_mass.assumedCellDwInFgC*1e-15))),1:S, 50, '.', 'b')
    % scatter(flip(Data_Evol_20_wo_Lysobacter_end(:,i)),1:S_wo_Lysobacter, 50, '.', 'b')
end
...
writetable(ind_syncom_test, 'Data/SynCom21-SynCom20-mattest.csv');
...
%comment those lines if not normilized
% mean_obs_biomass_21_renorm(11,:) = 0;
% mean_obs_biomass_21_renorm = mean_obs_biomass_21_renorm.*21/20;
