...
for nb_iter_tot = 1:1
    ...
    Data_Evol = readtable(strcat('Data/','abund_cfu_se.xlsx'), 'Sheet', 3, 'Range','26:47', 'Format','auto');%Data in liquid from Clara's experiments
    ...
    while p < N
        ...
        for k = 1:(num_steps)
            ...
            for j = 1:(lag)
                sol = ode45(@(t, y) fun_CF_Death_Pred(t, y, kappa_mat, CrossFeed_Mat_Temp, Mat_kappa_3, Resource_Matrix_cand, Threshold_CF, Threshold_death, Threshold_Pred, Death_Mat_Temp, death_rate, Pred_Mat_Lyso, yield_Pred, S, Lag_time_Cons_cand, Lag_time_Pred, nb_Res, Hill_CF_coeff, Hill_Pred_coeff), tspan,  mat_y_0, opts_1); %Multiple resource groups
                z_temp = deval(sol, Time_step);
                X = z_temp(mod(1:S*3,3) == 1, :); %Species'biomass
                P = z_temp(mod(1:S*3,3) == 2, :); %Complexes'biomass
                X = X + P;
    ...
    sol = ode45(@(t, y) fun_CF_Death_Pred(t, y, kappa_mat, CrossFeed_Mat_Temp, Mat_kappa_3, Resource_Matrix, Threshold_CF, Threshold_death, Threshold_Pred, Death_Mat_Temp, death_rate, Pred_Mat_Lyso, yield_Pred, S, Lag_time_Cons, Lag_time_Pred, nb_Res, Hill_CF_coeff, Hill_Pred_coeff), tspan,  mat_y_0, opts_1); %Multiple resource groups
    z_temp = deval(sol, Time_step);
    X = z_temp(mod(1:S*3,3) == 1,:); %Species'biomass
    P = z_temp(mod(1:S*3,3) == 2,:); %Complexes'biomass
    X = X + P;
...
save(strcat(cd, '/Data/', 'Zenodo_data_to_save_liquid_multiple_resources'), 'Zenodo_data_to_save_liquid_multiple_resources')