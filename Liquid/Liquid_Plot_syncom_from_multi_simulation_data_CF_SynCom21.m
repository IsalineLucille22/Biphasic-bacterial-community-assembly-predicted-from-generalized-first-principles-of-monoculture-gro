...
nb_res = 1; %In liquid, we assume a unique resource
...
%mean_y_0(4) = 0; %Some species disapear 
...
sim_max_biomass = zeros(nb_data_set, 1);
for i = 1:nb_data_set
    sim_max_biomass(i,:) = max(sum(exp_biomass(:,:,i)));
end

%repeat calculation of the measured abundances
obs_max_biomass = zeros(nb_rep, 1);

for i = 1:nb_rep
    obs_max_biomass(i,:) = max(sum(Measured_Abund(:,:,i)));
end
...
y = [mean(sim_max_biomass); mean(obs_max_biomass)];
...
hold on
for j = 1:nb_data_set
    scatter(X(1), sim_max_biomass(j),50,'k','.')
end
hold on
for j = 1:nb_rep
 scatter(X(2), obs_max_biomass(j),50,'k','.')
end
...
for i = 1:nb_rep 
    ...
    for k = 1:S
        ...
        Evol_mass = A(:,k);
        Evol_mass(Evol_mass <= 0) = 1e-08; %The minimum biomass should be higher than the tolerance 
        lag_time = 0;
