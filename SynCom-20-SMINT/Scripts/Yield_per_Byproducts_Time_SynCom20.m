...
%Be carful to change the line 26 of the function fun_CF_Death to dy_vect =
%z(:,1).*sum(CrossFeed_Mat.*((T.*z(:,3))'./(z(:,3)' + K_S_Waste)),2); to
%get the growth due to byproducts consumption
...
Data_Evol = readtable(strcat(dataFolder,'S20_S21_abs_abund_cfu_Senka.xlsx'), 'Sheet', 3, 'Range','1:21', 'Format','auto'); %Test with the new SynCom 20
mu_max_dist = table2array(Parameters_Senka_mu_max(:,7:8));
S = height(Data_Evol);
Time_step = [0 1 3 7 10 21]*24; %[0 1 3 7 10 15 21 22]*24; 
...
nb_rep_sim = nb_data_set;%2*nb_rep;
...
for zz = 1:nb_rep_sim    
    ...
    Death_Mat = data_to_save(ind_sim).Death_Mat_Temp; 
    ...
    yield_Pred = 0;% No predation in SynCom20
    ...
    for i = 1:nb_replicates
        ...
        for k = 1:n_exp
            ...
            sol = ode45(@(t, y) fun_CF_Death(t, y, kappa_mat, CrossFeed_Mat, Mat_kappa_3, Resource_Matrix, Threshold_CF, Threshold_Pred, Death_Mat, death_rate, S, Lag_time_Cons, Lag_time_Pred, nb_Res, 10, 10),...
                tspan,  mat_y_0, opts_1); %Multiple resource groups
    ...
    for i = 1:nb_time_step_BP - 1
        ...
        for j = 1:S
            ...
            temp(j,:) = (P_BP(j,i + 1) - P_BP(j,i)).*(X_BP(j, i)*...
                CrossFeed_Mat(j,:).*((T_temp.*W_BP(:,i))'./(W_BP(:,i)' + K_S_Waste(j,:))))...
...
FigName = strcat(iFolderName, name_Exp, 'mean-byproduct-strain-yields-per-time-SynCom20.pdf');
