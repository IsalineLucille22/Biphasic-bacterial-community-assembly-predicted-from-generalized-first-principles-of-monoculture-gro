...
%algorithm to it the co-culture interactions. The fitted parameters are the
...
Zenodo_data_to_save_liquid_multiple_resources = struct();
% parfor nb_iter_tot = 1:10    
for nb_iter_tot = 1:1 %1:1 loop to test
   
    %Loadind data
    Parameters_set = readtable(strcat(dataFolder,'MergedData.xlsx'), 'Sheet', 8,'Range','48:69', 'Format','auto');
    Parameters_Senka_mu_max = readtable(strcat(dataFolder,'MergedData.xlsx'), 'Sheet', 9, 'Range','118:139', 'Format','auto');%%readtable(strcat('Data/','MergedData.xlsx'), 'Sheet', 8, 'Range','48:69', 'Format','auto');%
    Parameters_Senka_Lag_time = readtable(strcat(dataFolder,'MergedData.xlsx'), 'Sheet', 9, 'Range','142:163', 'Format','auto');
    Data_Evol = readtable(strcat(dataFolder,'abund_cfu_se.xlsx'), 'Sheet', 3, 'Range','26:47', 'Format','auto');%Data in liquid from Clara's experiments
    ...
    %Turn 7 into 5
    yield_vect = table2array(Parameters_set(1:S,7)); %For now yield in soil, change it
    name = string(table2array(Parameters_set(1:S,1)));%string(table2array(table(2:22,3)));
    
    mu_max_dist = table2array(Parameters_Senka_mu_max(:,7:8)); %max(table2array(Parameters_Senka_mu_max(:,3)), 0.01); %Log-normal Parameters_Senka_mu_max(:,7:8)
    mu_max_vect = mu_max_dist(:,1);
    Res_Percentage = 1;%table2array(Parameters_Senka_mu_max(:,9)); %table2array(Parameters_Senka_mu_max(:,4)); %Percentage of resources that can be consumed
    ...
    t_0 = 0; %Time 0
    nb_Res = 1; %Number of resources (group of resources)
    nb_Res_max = 1;
    CrossFeed_Mat = lognrnd((log(0.8) + mu_max_dist(:,1)).*ones(S,S), 0*mu_max_dist(:,2).*ones(S,S));%max(normrnd(0.8*mu_max_dist.*ones(S,S), 0.05.*ones(S,S)), 0);%.val val% of zeros. ConsumerxProducer
    ...
    Resource_Matrix = Resource_Matrix.*lognrnd(-mu_max_dist(:,2).^2./2.*ones(S, nb_Res), mu_max_dist(:,2).*ones(S, nb_Res));%Resource_Matrix.*max(normrnd(ones(S, nb_Res), 0.05.*ones(S, nb_Res)), 0.01);%Resource_Matrix.*max(normrnd(ones(S, nb_Res), 0.05.*ones(S, nb_Res)), 0.01);%Resource_Matrix.*max(normrnd(ones(S, nb_Res), mu_max_dist(:,2)./mu_max_dist(:,1).*ones(S, nb_Res)), 0.01);%Centered on mu_max because multiplied mby mu_max so depend on the measured mu_max.
    R = R_0*15*10^(-3); %based on the observed values
    ...
    mat_y_0 = [mat_y_0 R];
    mu_max_vect_temp = exp(mu_max_vect + mu_max_dist(:,2).^2./2);%mu_max_vect;%
    ...
    ind_Lyso = 6;
    Pred_Mat_Lyso(ind_Lyso, Prey_num) = 1e-03; %Predation rate randomly chosen
        
    beta = 0.5*1e2; 
    num_steps = 100;
    nb_loops = 3; %Number of different loops (CF, predation. resource, nb of resources)
    ...
    while p < N
        ...
        for k = 1:num_steps
            ...
            for j = 1:lag
                sol = ode45(@(t, y) fun_CF_Death_Lyso(t, y, kappa_mat, CrossFeed_Mat_Temp_cand, Mat_kappa_3, Resource_Matrix, Threshold_CF_cand, Threshold_death, Threshold_Pred, Death_Mat_Temp, death_rate, Pred_Mat_Lyso, yield_Pred, S, Lag_time_Cons, Lag_time_Pred, nb_Res, Hill_CF_coeff_cand, Hill_Pred_coeff), tspan,  mat_y_0, opts_1); %Multiple resource groups
        ...
        for k = 1:num_steps
            ...
            for j = 1:lag
                sol = ode45(@(t, y) fun_CF_Death_Lyso(t, y, kappa_mat, CrossFeed_Mat_Temp, Mat_kappa_3, Resource_Matrix, Threshold_CF, Threshold_death_cand, Threshold_Pred, Death_Mat_Temp_cand, death_rate, Pred_Mat_Lyso, yield_Pred, S, Lag_time_Cons, Lag_time_Pred, nb_Res, Hill_CF_coeff, Hill_Pred_coeff_cand), tspan,  mat_y_0, opts_1); %Multiple resource groups
                z_temp = deval(sol, Time_step);
                X = z_temp(mod(1:S*3,3) == 1,:); %Species'biomass
                P = z_temp(mod(1:S*3,3) == 2,:); %Complexes'biomass
                X = X + P;
        ...
        for k = 1:(num_steps)
            ...
            for j = 1:(lag)
                sol = ode45(@(t, y) fun_CF_Death_Lyso(t, y, kappa_mat, CrossFeed_Mat_Temp, Mat_kappa_3, Resource_Matrix_cand, Threshold_CF, Threshold_death, Threshold_Pred, Death_Mat_Temp, death_rate, Pred_Mat_Lyso, yield_Pred, S, Lag_time_Cons_cand, Lag_time_Pred, nb_Res, Hill_CF_coeff, Hill_Pred_coeff), tspan,  mat_y_0, opts_1); %Multiple resource groups
                z_temp = deval(sol, Time_step);
                X = z_temp(mod(1:S*3,3) == 1, :); %Species'biomass
                P = z_temp(mod(1:S*3,3) == 2, :); %Complexes'biomass
                X = X + P;
    ...
    disp(acceptance_ratio_temp)
    disp(liste_nrj_tot_temp)
    ...
    sol = ode45(@(t, y) fun_CF_Death_Lyso(t, y, kappa_mat, CrossFeed_Mat_Temp, Mat_kappa_3, Resource_Matrix, Threshold_CF, Threshold_death, Threshold_Pred, Death_Mat_Temp, death_rate, Pred_Mat_Lyso, yield_Pred, S, Lag_time_Cons, Lag_time_Pred, nb_Res, Hill_CF_coeff, Hill_Pred_coeff), tspan,  mat_y_0, opts_1); %Multiple resource groups
    z_temp = deval(sol, Time_step);
    X = z_temp(mod(1:S*3,3) == 1,:); %Species'biomass
    P = z_temp(mod(1:S*3,3) == 2,:); %Complexes'biomass
    X = X + P;
    ...
    for iFig = 1:length(FigList)
        ...
        FigName = strcat(iFolderName, FigName, 'Liquid_System');
        set(0, 'CurrentFigure', FigHandle);
        saveas(FigHandle, FigName, 'pdf');
    end

    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).CrossFeed_Mat_Temp = CrossFeed_Mat_Temp;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).Pred_Mat_Lyso = Pred_Mat_Lyso;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).Threshold_CF = Threshold_CF;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).Threshold_death = Threshold_death;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).Threshold_Pred = Threshold_Pred;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).Death_Mat_Temp = Death_Mat_Temp;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).Resource_Matrix = Resource_Matrix;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).Lag_time_Pred = Lag_time_Pred;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).Lag_time_Cons = Lag_time_Cons;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).Mat_kappa_3 = Mat_kappa_3;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).kappa_mat = kappa_mat;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).R = R;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).CrossFeed_Mat_init = CrossFeed_Mat_init;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).Threshold_CF_init = Threshold_CF_init;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).Threshold_death_init = Threshold_death_init;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).Death_Mat_init = Death_Mat_init;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).Resource_Matrix_init = Resource_Matrix_init;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).Lag_time_Pred_init = Lag_time_Pred_init;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).Lag_time_Cons_init = Lag_time_Cons_init;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).Mat_kappa_3_init = Mat_kappa_3_init;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).kappa_mat_init = kappa_mat_init;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).R_init = R_init;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).Struct_Res_Mat = Struct_Res_Mat;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).Struct_Death_Mat = Struct_Death_Mat;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).Struct_CrossFeed_Mat = Struct_CrossFeed_Mat;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).Cov_Mat_CF = Cov_Mat_CF;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).Cov_Mat_Death = Cov_Mat_Death;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).Cov_Mat_Res = Cov_Mat_Res;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).Cov_Mat_LT = Cov_Mat_LT;
    Zenodo_data_to_save_liquid_multiple_resources(nb_iter_tot).death_rate = death_rate;
end

save(strcat(dataFolder, 'Zenodo_data_to_save_liquid_multiple_resources'), 'Zenodo_data_to_save_liquid_multiple_resources')