...
z = z';%[S_i, P_i, W_i]
k = Hill_CF;%10;
T = (1./(1 + (Threshold_CF(1)./z(:,3)).^k));%(1./(1 + (Threshold./z(:,3)).^k));
T(isnan(T)) = 0;
k = Hill_Pred;%10;
...
T_pred = (1./(1 + (Threshold_Pred(1)./z(6,1)).^5)).*(1./(1 + (1e-05./z(:,1)).^5));%(1./(1 + (Threshold_Pred(1)./z(:,1)).^10));
...
dx_vect =  z(:,1).*sum(kappa(:,2).*((t > Lag_time_Cons).*Resource_Matrix).*(r'./(r' + K_S_res)),2)...
    ...
    - z(:,1).*(T_long_Term_death.*death_rate);
dy_vect = zeros(S,1); %z(:,1).*sum(CrossFeed_Mat.*((T.*z(:,3))'./(z(:,3)' + K_S_Waste)),2);%Growth induced by byproducts consumption
