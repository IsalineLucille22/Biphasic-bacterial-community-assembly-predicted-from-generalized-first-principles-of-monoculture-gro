...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    nu_2 = min(1, 2*nu);
    N  = S_consumer;
    Loop_Mat_Temp_vec = diag(Loop_Mat_Temp);
    w_dir  = W_n_Cov_Mat;
    w_norm = norm(w_dir) + eps;
    w_dir  = w_dir / w_norm;
    A = eye(N) + nu_2*(accept_temp(k) - 0.234)*(w_dir*w_dir.');
    Sigma_new = Cov_Mat * A * Cov_Mat.';
    Sigma_new = (Sigma_new + Sigma_new.')/2;        
    Sigma_new = Sigma_new + 1e-6*eye(N);           
    Cov_Mat = chol(Sigma_new, 'lower');
    W_n_Cov_Mat = zeros(N,1);
    Loop_Mat_Temp_cand_vec = Loop_Mat_Temp_vec;      
    while all(W_n_Cov_Mat == 0)
        W_n_Cov_Mat = 0.1*randn(N,1);
        step    = Cov_Mat * W_n_Cov_Mat;
        tempvec = Loop_Mat_Temp_vec + step;
        Loop_Mat_Temp_cand_vec = max(tempvec, 0);
        idx_neg = (tempvec < 0);
        W_n_Cov_Mat(idx_neg) = step(idx_neg) - tempvec(idx_neg);
        idx_high = (Loop_Mat_Temp_cand_vec > max_val);
        Loop_Mat_Temp_cand_vec(idx_high) = Loop_Mat_Temp_vec(idx_high);    
        W_n_Cov_Mat(idx_high) = 0;
    end
    Loop_Mat_Temp = diag(Loop_Mat_Temp_vec);
    Loop_Mat_Temp_cand = diag(Loop_Mat_Temp_cand_vec);

    % Modification thresholds
    theta_T_Loop = theta_T_Loop*(eye(S_consumer) + nu_2*(accept_temp(k) - 0.234)*( W_n_thresh* W_n_thresh')/norm( W_n_thresh)^2)*theta_T_Loop';
    ...
    logtheta = log(theta_LT_Loop) + nu*(accept_temp(k) - 0.234);%
