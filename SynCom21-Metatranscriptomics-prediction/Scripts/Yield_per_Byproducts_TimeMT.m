...
%Be carful to change the line 26 of the function fun_CF_Death_Lyso to dy_vect = z(:,1).*sum(CrossFeed_Mat.*((T.*z(:,3))'./(z(:,3)' + K_S_Waste)),2); to
...
name_Exp = 'Zenodo_SynCom21MTByproduct';
...
Data_Evol = readtable(strcat(dataFolder,'SSC21_genera_relative-abundances.xlsx'), 'Sheet', 4, 'Range','1:22', 'Format','auto'); %Soil microcosm 21, Meta-transcriptomic data
mu_max_dist = table2array(Parameters_Senka_mu_max(:,7:8));
S = height(Data_Evol);
Time_step = [0 12 22 38 70 168 504];%Metatranscriptomics'data
...
for zz = 1:nb_rep_sim    
    ...
    for i = 1:nb_time_step_BP - 1
        temp = zeros(S);
        for j = 1:S
            T_temp = (1./(1 + (Threshold_CF(1)./W_BP(:,i)).^10)); %(1./(1 + (Threshold_Pred(1)./X_BP(6,i)).^5)).*(1./(1 + (1e-05./X_BP(:,i)).^5));%
