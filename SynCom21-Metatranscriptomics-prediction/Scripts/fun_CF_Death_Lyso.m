...
dy_vect = zeros(S,1); %z(:,1).*sum(CrossFeed_Mat.*((T.*z(:,3))'./(z(:,3)' + K_S_Waste)),2);%yield_Pred.*z(:,1).*sum(((t > Lag_time_Pred).*Pred_Mat).*(T_pred.*z(:,1))'./(z(:,1)' + K_S_Pred), 2);%z(:,1).*sum(CrossFeed_Mat.*((T.*z(:,3))'./(z(:,3)' + K_S_Waste)),2);%Byproduct growth, just for information. Used in the Yield_per_Byproducts_Time.m script
