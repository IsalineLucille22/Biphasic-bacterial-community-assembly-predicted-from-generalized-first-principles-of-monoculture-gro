...
Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor= struct(); %To avoid overwriting, change the name of the save variable at the last line of the script. Name it accoridng to this structure name.
...
for nb_iter_tot = 1:1 %1:1 loop to test
    ...
    Data_Evol = readtable(strcat(dataFolder,'S20_S21_abs_abund_cfu_Senka.xlsx'), 'Sheet', 12, 'Range','1:22', 'Format','auto');
    Data_Evol_test = Data_Evol;
    Time_step = [0 1 3 7 10 21]*24; %[0 1 3 7 10 21]*24; %Measured time step in hours %CONTROL IT
    ...
    Prey_num = [1:(ind_Lyso - 1), (ind_Lyso + 1):S];%[7 11 15 16 17 21];%[11 15 17 21];%[11 15 17];%[11 17 21];%[3 7 11 13 15 17 20 21];
    ...
    Lag_time_Pred(ind_Lyso, Prey_num) = 0;
    ...
    while p < N
        ...
        W_n_thresh_death = normrnd(zeros(S,1), ones(S,1)); 
        ...
        for k = 1:num_steps
            ...
            for j = 1:lag
                ...
                [accept_temp, W_n_thresh_death, W_n_Cov_Mat_death, Death_Mat_Temp_cand, Death_Mat_Temp, Threshold_death_cand, Threshold_death, Struct_Death_Mat, liste_nrj_tot, liste_nrj_pred, acceptance_ratio_tot, acceptance_ratio_death, p, r, theta_T_death, ~, nu, Cov_Mat] = ...
                    ...
                    acceptance_ratio_tot, acceptance_ratio_death, S, S, X, Measured_Abund, StackPlotTot, StackPlot_Meas, beta, theta_T_death, 0, nu, Cov_Mat, Cov_Lt_eye, max_val(:,2), nb_rep, LT_CF_Death, LT_CF_Death, Weight_species, weight_day_seven, Temp);
        ...
        Cov_Mat = eye(S*nb_Res); %0.1*eye(S*nb_Res); %Cov_Mat_Res; %
        ...
        for k = 1:(num_steps)
            ...
            for j = 1:(lag)
                ...
                [accept_temp, W_n_LT_Pred, W_n_thresh_Pred, W_n_Cov_Mat_Pred, Pred_Mat_Lyso_cand, Pred_Mat_Lyso, T_cand, T, Struct_Res_Mat, liste_nrj_tot, liste_nrj_res, acceptance_ratio_tot, acceptance_ratio_pred, p, l, ~, nu, Cov_Mat] = ...
                    ...
                    acceptance_ratio_tot, acceptance_ratio_pred, S, S, X, Measured_Abund, StackPlotTot, StackPlot_Meas, beta, eye(S), nu, Cov_Mat, Cov_Mat_LT, max_val(:,3), nb_rep, LT_CF_Death, LT_CF_Death, Weight_species, weight_day_seven, Temp);
    ...
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).CrossFeed_Mat_Temp = CrossFeed_Mat_Temp;   
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).Pred_Mat_Lyso = Pred_Mat_Lyso;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).Threshold_Pred = Threshold_Pred;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).Threshold_CF = Threshold_CF;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).Threshold_death = Threshold_death;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).Death_Mat_Temp = Death_Mat_Temp;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).Resource_Matrix = Resource_Matrix;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).Lag_time_Pred = Lag_time_Pred;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).Lag_time_Cons = Lag_time_Cons;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).Mat_kappa_3 = Mat_kappa_3;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).kappa_mat = kappa_mat;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).R = R;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).CrossFeed_Mat_init = CrossFeed_Mat_init;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).Threshold_CF_init = Threshold_CF_init;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).Pred_Mat_Lyso_init = Pred_Mat_Lyso_init;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).Threshold_death_init = Threshold_death_init;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).Death_Mat_init = Death_Mat_init;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).Resource_Matrix_init = Resource_Matrix_init;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).Lag_time_Pred_init = Lag_time_Pred_init;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).Lag_time_Cons_init = Lag_time_Cons_init;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).Mat_kappa_3_init = Mat_kappa_3_init;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).kappa_mat_init = kappa_mat_init;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).R_init = R_init;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).Struct_Res_Mat = Struct_Res_Mat;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).Struct_Death_Mat = Struct_Death_Mat;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).Struct_CrossFeed_Mat = Struct_CrossFeed_Mat;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).Struct_Pred_Mat_Lyso = Struct_Pred_Mat_Lyso;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).Cov_Mat_CF = Cov_Mat_CF;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).Cov_Mat_Death = Cov_Mat_Death;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).Cov_Mat_Res = Cov_Mat_Res;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).Cov_Mat_LT = Cov_Mat_LT;
    Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor(nb_iter_tot).death_rate = death_rate;
end

save(strcat(dataFolder, 'Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor'), 'Zenodo_data_to_save_SynCom21_Soil_only_Lyso_Parfor')