...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    Loop_Mat_Temp_vec = reshape(Loop_Mat_Temp, [], 1);   
    w_dir = W_n_Cov_Mat;
    w_norm = norm(w_dir) + eps;
    w_dir  = w_dir/w_norm;    
    % Robust adaptive Metropolis (RAM-style) rank-1 adaptation matrix
    A = eye(N) + nu_2*(accept_temp(k) - 0.234)*(w_dir*w_dir.');
    Sigma_new = Cov_Mat*A*Cov_Mat.';
    Sigma_new = (Sigma_new + Sigma_new.')/2;          
    Sigma_new = Sigma_new + 1e-4*eye(N);            
    Cov_Mat = chol(Sigma_new, 'lower');
    W_n_Cov_Mat = 0.1*randn(N,1);
    step = Cov_Mat*W_n_Cov_Mat;
    temp_vec = Loop_Mat_Temp_vec + step;
    Loop_Mat_Temp_cand_vec = max(temp_vec, 0);
    idx_neg = (temp_vec < 0);
    W_n_Cov_Mat(idx_neg) = step(idx_neg) - temp_vec(idx_neg);
    Loop_Mat_Temp_cand = reshape(Loop_Mat_Temp_cand_vec, S_consumer, S_consumed);
    High_val = (Loop_Mat_Temp_cand > max_val);
    Loop_Mat_Temp_cand(High_val) = Loop_Mat_Temp(High_val);
